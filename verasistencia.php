...
<small><font color="red">* Al eliminar una Asistencia, igual se eliminan las asistencias la que le pertenece</font></small>
<table width="100%">
		<tr>
		<td class="down1">
		<strong><center> LISTA DE ASISTENCIA </center></strong>
		</td>
		</tr>
...
<table id="table" cellspacing="0" width="100%">
 ...
 if(mysqli_num_rows($rs) !=0 ){
	while($row=mysqli_fetch_assoc($rs)){
		...
		echo '<tr class="'.$class.'">';
		echo '<td>';
	    echo '<img src="imagenes/actions-edit.png" onclick="editar('.$row['id_asistencia'].')"/>  ';
		echo '<img src="imagenes/actions-delete.png" onclick="eliminar('.$row['id_asistencia'].')"/>';
		echo '</td>';
		echo '<td>'.$contador.'</td>';
